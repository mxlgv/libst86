...
void test_main(void)
{
    uintptr_t* const addrs_expected = malloc(depth_requested * sizeof(*addrs_expected));
    assert(addrs_expected);

    uintptr_t* const addrs = malloc(depth_requested * sizeof(*addrs));
    assert(addrs);

    const int depth_expected = backtrace((void**)addrs_expected, depth_requested);
    assert(depth_expected);

    char** const strs_expected = backtrace_symbols((void**)addrs_expected, depth_expected);
    assert(strs_expected);

    char* tmpbuf_expected;
    size_t dummy_size;
    FILE* const memfile_expected = open_memstream(&tmpbuf_expected, &dummy_size);
...
int main(void)
{
    ...
    printf("frame_addr: %p == %p\n", st86_get_frame_addr(), __builtin_frame_address(0));
